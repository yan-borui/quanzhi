...
class StateBindingSystem:
    ...
    def bind_state(
        ...
        if auto_unbind_old and key in self.bindings:
            ...
            if old_target_id != target_id:
                old_target_name = (
                    old_binding.target.get_name()
                    if hasattr(old_binding.target, "get_name")
                    else str(old_binding.target)
                )
                emit(
                    f"[状态绑定] {source_name} 的 '{skill_name}' 从 {old_target_name} 解绑"
                )
                self.unbind_state(source, skill_name)

        # 创建新绑定
        binding = StateBinding(
            skill_name, source, target, on_bind, on_unbind, state_name
        )
        ...
        emit(f"[状态绑定] {source_name} 的 '{skill_name}' 绑定到 {target_name}")
    ...
    def unbind_state(self, source: Any, skill_name: str) -> bool:
        ...
        source_name = source.get_name() if hasattr(source, "get_name") else str(source)
        emit(f"[状态绑定] {source_name} 的 '{skill_name}' 已解绑")
    ...
    def unbind_all_from_target(self, target: Any):
        ...
        target_name = target.get_name() if hasattr(target, "get_name") else str(target)
        emit(
            f"[状态绑定] {target_name} 身上的所有绑定已清除 (数量: {len(keys_to_unbind)})"
        )
    ...
    def unbind_all_from_source(self, source: Any):
        ...
        if keys_to_unbind:
            source_name = (
                source.get_name() if hasattr(source, "get_name") else str(source)
            )
            emit(
                f"[状态绑定] {source_name} 的所有绑定已清除 (数量: {len(keys_to_unbind)})"
            )
