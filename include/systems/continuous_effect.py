...
class ContinuousEffect:
    ...
    def check_removal_condition(self, target: Any, event: str) -> bool:
        ...
        # 检查预定义条件
        if (
            self.removal_condition == RemovalCondition.ON_MOVEMENT
            and event == "movement"
        ):
            return True
            ...
            self.removal_condition == RemovalCondition.ON_CONTROL and event == "control"
        ):
            return True
...
class ContinuousEffectSystem:
    ...
    def __init__(self):
        ...
        # 存储每个位置块的持续效果列表: block_id -> List[ContinuousEffect]
        self.block_effects: Dict[int, List[ContinuousEffect]] = {}
    ...
    def add_block_effect(self, block_id: int, effect: ContinuousEffect):
        """
        为位置块添加持续效果。

        块效果在每回合对块内所有角色触发，适合燃烧瓶这类地面持续伤害。
        """
        if block_id not in self.block_effects:
            self.block_effects[block_id] = []
    ...
    def trigger_block_effects(self, block_id: int, targets: Iterable[Any]):
        """
        触发指定位置块上的所有持续效果。

        Args:
            block_id: 位置块 ID
            targets: 当前处于该块内的角色列表
        """
        if block_id not in self.block_effects:
            return

        effects = self.block_effects[block_id]
        if not effects:
            return
    ...
    def get_block_effects(self, block_id: int) -> List[ContinuousEffect]:
        """获取指定位置块的所有持续效果。"""
        return self.block_effects.get(block_id, []).copy()
    ...
    def has_effect(self, target: Any, effect_name: str) -> bool:
        ...
        return any(
            effect.name == effect_name for effect in self.character_effects[target_id]
        )
    ...
    def get_effect_count(self, target: Any, effect_name: str) -> int:
        ...
        return sum(
            1
            for effect in self.character_effects[target_id]
            if effect.name == effect_name
        )
