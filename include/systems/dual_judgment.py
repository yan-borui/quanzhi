...
class DualJudgmentSystem:
    ...
    def register_skill_rule(
        self, skill_name: str, rule_func: Callable[[Any, Any], Optional[JudgmentResult]]
    ):
        """
        注册技能的特殊判定规则
        ...
        emit(f"[判定系统] 为技能 '{skill_name}' 注册了特殊规则")

    def unregister_skill_rule(self, skill_name: str):
        """取消注册技能的特殊判定规则"""
        if skill_name in self.skill_rules:
            del self.skill_rules[skill_name]
            emit(f"[判定系统] 移除了技能 '{skill_name}' 的特殊规则")

    def judge(
        self, initiator: Any, target: Any, skill_name: str = ""
    ) -> JudgmentResult:
        """
        执行双人判定
        ...
        if skill_name and skill_name in self.skill_rules:
            ...
            if forced_result is not None:
                emit(
                    f"[判定系统] 技能 '{skill_name}' 应用特殊规则，结果: {forced_result.value}"
                )
    ...
    def _normal_rps_judgment(
        ...
        """
        initiator_name = (
            initiator.get_name() if hasattr(initiator, "get_name") else str(initiator)
        )
    ...
    def create_auto_win_rule(
        self, winner: str = "initiator"
    ) -> Callable[[Any, Any], Optional[JudgmentResult]]:
        """
        创建自动获胜规则
    ...
    def create_conditional_rule(
        self, condition_func: Callable[[Any, Any], bool], success_result: JudgmentResult
    ) -> Callable[[Any, Any], Optional[JudgmentResult]]:
        """
