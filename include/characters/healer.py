...
class Healer(Character):
    ...
    def add_control(self, control_name: str, stacks: int = 1):
        ...
        # 记录立盾期间受到的控制（非无害控制）
        if self.shield_hp > 0 and control_blocks_action(control_name):
    ...
    def _auto_clear_shield_controls(self):
        ...
        for control_name in controls_to_clear:
            if self.has_control(control_name):
                ...
                emit(f"{self.name} 的立盾破碎，自动解除了 {control_name} 控制")
    ...
    def _big_heal_effect(self, caster: Character, target: Optional[Character]) -> bool:
        ...
        if target is self:
            ...
            emit(f"{self.name} 获得减伤状态，剩余 {self.damage_reduction_turns} 回合")
        return True

    def _small_heal_effect(
    ...
    def _shield_effect(self, caster: Character, target: Optional[Character]) -> bool:
        ...
        if isinstance(target, Healer):
            if target.shield_hp > 0:
                target.shield_hp += 3
                emit(
                    f"{target.get_name()} 的立盾增加了3点血量，当前立盾血量: {target.shield_hp}"
                )
            else:
                target.shield_hp = 3
                target._shield_controls.clear()
                emit(f"{target.get_name()} 获得了立盾，血量: {target.shield_hp}")
        else:
            # 对非治疗师目标使用：生成立盾效果（使用累积效果）
            target.add_modifier("立盾", 3)
            emit(f"{target.get_name()} 获得了立盾效果")
        return True
...
HEALER_SKILLS_DATA = {
    "大血包": {
        ...
        "cooldown": 2,
        "damage": -12,
        "effect": "治疗；对自己使用时附加减伤状态（+2回合，上限2回合，减伤50%）",
        "common": True,
    },
    "小血包": {
        "name": "小血包",
        "cooldown": 1,
        "damage": -6,
        "effect": "治疗；对自己使用时附加减伤状态（+1回合，上限2回合，减伤50%）",
    },
    "套盾": {
        "name": "套盾",
        "cooldown": 1,
        "damage": 0,
        "effect": "生成立盾（3血量），叠加回复3血量；伤害优先扣立盾，溢出伤害不传递；盾碎自动解控",
    },
