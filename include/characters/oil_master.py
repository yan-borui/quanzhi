...
class OilMaster(Character):
    ...
    def use_skill_on_target(self, skill_name: str, target: Character):
        ...
        if skill_name == "倒你脸上" and self.oil_pots <= 0:
            emit("本回合已倒过油，无法再次使用倒你脸上")
            return

        if skill_name == "倒你脸上" and self.oil_pot_count <= 0:
            emit(f"{self.name} 没有可用的油锅，无法使用倒你脸上")
            return
    ...
    def drink_oil(self, drinker: Character) -> bool:
        ...
        if self.oil_pot_count <= 0:
            emit(f"{self.name} 没有可用的油锅")
            return False
        self.oil_pot_count -= 1
        drinker.heal(3)
        emit(
            f"{drinker.get_name()} 喝了 {self.name} 的油，HP+3，剩余油锅: {self.oil_pot_count}"
        )
        return True
    ...
    def _pot_effect(self, caster: Character, target: Optional[Character]) -> bool:
        # 瞬发技能：直接增加自身油锅计数+1
        self.oil_pot_count += 1
        emit(f"{self.name} 的油锅计数 +1，当前油锅: {self.oil_pot_count}")
        return True
...
OIL_MASTER_SKILLS_DATA = {
    ...
    "一锅油": {
        ...
        "cooldown": 3,
        "damage": 0,
        "effect": "瞬发，自身油锅计数+1；油锅可被任何角色喝（HP+3）",
    },
    "倒你脸上": {
        "name": "倒你脸上",
        "cooldown": 0,
